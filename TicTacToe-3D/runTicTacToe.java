...
public class runTicTacToe {
	...
	public static List<positionTicTacToe> board = new ArrayList<>();
	private humanTicTacToe ai1;
	...
	public runTicTacToe()
	{
		...
		do{
			...
			System.out.println("Pick difficulty Easy press e, Medium press m, Hard press h, Extreme x ");
		
			c = sc.next().charAt(0);
			//System.out.printf("%d",pick);
			if(c != 'h' && c != 'e' && c != 'x' && c != 'm'){
				System.out.println("Not a level option pick");
				checker = false;
				}
			}while(checker == false);
		...
		ai1 = new humanTicTacToe(1);
		ai2 = new aiTicTacToe(2,c);
	}
	public static List<positionTicTacToe> getTicTacToeBoard(){
		return board;
	}
	...
	{
		Scanner sc = new Scanner(System.in);
		System.out.println("First-F, Second-S or Random-R ; ");
		while(!sc.hasNext("[fF]")&&!sc.hasNext("[sS]") && !sc.hasNext("[Rr]")){
		...
		if(decide.equals("S"))
			...
			turn = rand.nextInt(2)+1; //1 = player1's turn, 2 = player2's turn, who go first is randomized 
			}
		int count = 1;
		//Random rand = new Random();
